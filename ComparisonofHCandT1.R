#Farnaz Fouladi
#12-02-2019
#Comparing shared SV between HC and T1
...
patients=c("AN81HC","AN81T1","AN40HC","AN40T1","AN703HC","AN703T1","AN34HC","AN34T1")
...
plot1<-ggplot(data=df,aes(x=Patient,y=percentBugShared3,color=Treatment))+geom_boxplot(outlier.shape = NA)+
  ...
  scale_x_discrete(labels=c("1","2","3","4"))+labs(y="Shared (%)",x="Patients")+
  annotate(geom = "text",x=c(1,2,3,4),y=c(67,67,67,67),label=c("adjusted p<0.01","adjusted p<0.01","adjusted p<0.01","adjusted p<0.01"),size=2)

pval<-vector()
pval[1]<-t.test(df[df$Patient=="34" & df$Treatment=="HC",]$percentBugShared3,
                df[df$Patient=="34" & df$Treatment=="T1",]$percentBugShared3)$p.value

pval[2]<-t.test(df[df$Patient=="40" & df$Treatment=="HC",]$percentBugShared3,
                df[df$Patient=="40" & df$Treatment=="T1",]$percentBugShared3)$p.value

pval[3]<-t.test(df[df$Patient=="81" & df$Treatment=="HC",]$percentBugShared3,
                df[df$Patient=="81" & df$Treatment=="T1",]$percentBugShared3)$p.value

pval[4]<-t.test(df[df$Patient=="703" & df$Treatment=="HC",]$percentBugShared3,
                df[df$Patient=="703" & df$Treatment=="T1",]$percentBugShared3)$p.value

p.adjust(pval,method = "BH")
pdf("ComparisonOfHCandT1.pdf",height = 5,width = 5)
print(plot1)
dev.off()
