...
pdf("pieSlurrybug_suppFigure.pdf",height = 10, width = 10)
par(mfrow=c(2,2))
for (i in 1:4)
{
...
#i=1
#meanPercentage<-c(as.numeric(format(df2[8,i],digit=4)),as.numeric(format(df2[9,i],digit=4)),as.numeric(format(df2[10,i],digit=4)))
#deviations<-c(as.numeric(format(df2[8,i+4],digits = 3)),as.numeric(format(df2[9,i+4],digits = 4)),as.numeric(format(df2[10,i+4],digits = 4)))
#slices<-paste(meanPercentage,"\u00b1",deviations,"%",sep="")
#labels<-c("Shared","Only in slurry","Only in mouse fecal pellet")
#labels<-paste(labels,"\n",slices)

#pdf("PieChartHumanSlurryMouseFeces.pdf",width=10, height=13)
