...
meanPercentage1<-c("67.91","13.80","18.29")
deviations<-c(as.numeric(format(df2[8,2],digits = 3)),as.numeric(format(df2[9,2],digits = 3)),as.numeric(format(df2[10,2],digits = 4)))
slices<-paste(meanPercentage1,"\u00b1",deviations,"%",sep="")
labels<-c("Shared","Only in human fecal sample","Only in slurry")
labels<-paste(labels,"\n",slices)

pdf("PieChartHumanFecalSlurry_rarefied.pdf",width=10, height=13)
